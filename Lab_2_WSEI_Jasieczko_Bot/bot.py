...
async def classify_lab2_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Obsługa nowej komendy /classify z Lab 2."""
    text = update.message.text.replace("/classify", "").strip()
    
    # 1. Rozbijamy tekst na argumenty klucz=wartość
    args_dict = {}
    ...
    # 2. Sprawdzamy, czy mamy wszystkie wymagane parametry
    required_keys = ["dataset", "method", "gridsearch", "run"]
    missing_keys = [k for k in required_keys if k not in args_dict]
    
    if missing_keys:
        await update.message.reply_text(
            f"Błąd! Brakuje parametrów: {', '.join(missing_keys)}.\n"
            "Przykład: /classify dataset=20news_group method=rf gridsearch=false run=1"
        )
        return

    # 3. Zczytanie wartości
    dataset = args_dict["dataset"]
    method = args_dict["method"]
    # Zmieniamy tekst "true"/"false" na pythonowego booleana
    gridsearch = True if args_dict["gridsearch"] == "true" else False 
    
    try:
        run_count = int(args_dict["run"])
    except ValueError:
        await update.message.reply_text("Błąd: Parametr 'run' musi być liczbą całkowitą.")
        return

    # Wysłanie potwierdzenia do użytkownika
    await update.message.reply_text(
        f"🚀 Rozpoczynam eksperyment (to może zająć sporo czasu!)...\n"
        f"Dataset: {dataset}\nMetoda: {method}\nGridSearch: {gridsearch}\nLiczba przebiegów: {run_count}"
    )

    # 4. Uruchamiamy ciężkie obliczenia w osobnym wątku, żeby bot nadal mógł odpowiadać na inne komendy!
    try:
        # Tę funkcję napiszemy w kolejnym kroku w pliku lab2_experiments.py
        raport = await asyncio.to_thread(
            lab2_experiments.run_experiment, 
            dataset, method, gridsearch, run_count
        )
        await update.message.reply_text(f"✅ Eksperyment zakończony!\n\n{raport}")
        
    except Exception as e:
        await update.message.reply_text(f"❌ Wystąpił błąd podczas obliczeń: {e}")
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler("classify", classify_lab2_command))
