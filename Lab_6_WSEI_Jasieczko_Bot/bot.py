...
def main():
    ...
    # Rejestracja obsługi zdjęć (reaguje na każdą wiadomość zawierającą obraz)
    app.add_handler(MessageHandler(filters.PHOTO, bot_commands.handle_photo))
    
    # --- Rejestracja komend z Lab 4 ---
    app.add_handler(CommandHandler("ner", bot_commands.ner_command))
    app.add_handler(CommandHandler("translate", bot_commands.translate_command))
    app.add_handler(CommandHandler("summarize", bot_commands.summarize_command))
    app.add_handler(CommandHandler("analyze_entities", bot_commands.analyze_entities_command))
    app.add_handler(CommandHandler("language_detect", bot_commands.language_detect_command))
