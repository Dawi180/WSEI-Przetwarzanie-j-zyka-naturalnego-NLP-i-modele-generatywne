...
# ==========================================
# FUNKCJE WYMAGANE PRZEZ LAB 06 (Moderacja)
# ==========================================

def analyze_sentiment_for_moderation(text: str) -> dict:
    """
    Analizuje sentyment tekstu dla bota moderacyjnego.
    """
    text_lower = text.lower()
    
    # Prosta heurystyka do oceny emocji
    negative_words = ['nienawidzę', 'głupi', 'idiota', 'złodzieje', 'śmieci', 'beznadziejny', 'rozczarowany', 'oszusci']
    positive_words = ['uwielbiam', 'najlepszy', 'super', 'polecam', 'zadowolony', 'dziękuję']
    
    neg_count = sum(1 for word in negative_words if word in text_lower)
    pos_count = sum(1 for word in positive_words if word in text_lower)
    
    if neg_count > pos_count:
        return {'sentiment': 'negative', 'confidence': 0.85, 'emotion': 'anger', 'sarcasm_detected': False}
    elif pos_count > neg_count:
        return {'sentiment': 'positive', 'confidence': 0.90, 'emotion': 'joy', 'sarcasm_detected': False}
    else:
        return {'sentiment': 'neutral', 'confidence': 0.60, 'emotion': 'neutral', 'sarcasm_detected': False}

def extract_moderation_entities(text: str) -> dict:
    """
    Wyciąga kluczowe encje z tekstu (osoby, organizacje, linki).
    """
    # Ekstrakcja za pomocą RegEx (wyłapywanie wzorców)
    urls = re.findall(r'(https?://[^\s]+)', text)
    emails = re.findall(r'[\w\.-]+@[\w\.-]+', text)
    phone_numbers = re.findall(r'(?:\+48)?\s?\d{3}[-\s]?\d{3}[-\s]?\d{3}', text)
