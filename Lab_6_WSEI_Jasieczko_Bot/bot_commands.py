...
# Importy Lab 6
import csv
import moderation_engine # Nasz nowy silnik!
...
async def translate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Wyłapywanie argumentów
    text_match = re.search(r'text="([^"]+)"', text_msg)
    target_match = re.search(r'target_lang=(\w+)', text_msg)
    source_match = re.search(r'source_lang=(\w+)', text_msg) # NOWE: Szukamy źródła

    if not target_match or not text_match:
        await update.message.reply_text("❌ Użycie: `/translate text=\"tekst\" [source_lang=es] target_lang=en`", parse_mode="Markdown")
        return

    text = text_match.group(1)
    target_lang = target_match.group(1).lower()
    
    # Jeśli użytkownik podał język, zapisujemy go. Jeśli nie, zostawiamy None.
    source_lang_input = source_match.group(1).lower() if source_match else None

    await update.message.reply_text("⏳ Uruchamiam lokalne modele tłumaczące (Hugging Face)...")
    
    # Przekazujemy nasz nowy argument do funkcji tłumaczącej
    translated_text, final_source_lang = await asyncio.to_thread(
        translation_tools.translate_text, text, target_lang, source_lang_input
    )
    
    raport = (f"**Source:** {final_source_lang}\n**Target:** {target_lang}\n\n**Translation:**\n{translated_text}")
    await update.message.reply_text(raport, parse_mode="Markdown")
...
async def moderate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Obsługa komendy /moderate 'tekst'"""
    # Sprawdzamy, czy użytkownik podał tekst po komendzie
    if not context.args:
        await update.message.reply_text("❌ Użycie: `/moderate <tekst do sprawdzenia>`", parse_mode="Markdown")
        return

    text_to_check = " ".join(context.args)
    user_id = str(update.effective_user.id)

    # Wysłanie wiadomości o ładowaniu
    status_msg = await update.message.reply_text("⏳ Uruchamiam systemy moderacyjne AI...")

    try:
        # Wywołujemy nasz potężny silnik z Etapu 3
        raport = await moderation_engine.process_message(text_to_check, user_id)
        
        # Edytujemy wiadomość, wklejając gotowy raport od Qwena
        await status_msg.edit_text(raport, parse_mode="Markdown")
    except Exception as e:
        await status_msg.edit_text(f"❌ Wystąpił błąd podczas moderacji: {e}")

async def mod_analytics_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Obsługa komendy /mod_analytics - generowanie biznesowego raportu z bazy CSV"""
    await update.message.reply_text("📊 Generowanie statystyk moderacyjnych...")
    ...
    try:
        ...
        with open(moderation_db.LOG_FILE, mode='r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                elif action == 'REJECT': rejected += 1
                elif action == 'BAN': banned += 1
                elif action == 'REVIEW': flagged += 1
                
        if total == 0:
            await update.message.reply_text("Brak danych w bazie logów.")
            return

        # Generujemy ładny raport
        raport = f"📊 **MODERATION ANALYTICS**\n"
        raport += f"========================\n"
        raport += f"Sprawdzone wiadomości: **{total}**\n\n"
        raport += f"✅ Zatwierdzone: {approved} ({(approved/total)*100:.1f}%)\n"
        raport += f"❌ Odrzucone: {rejected} ({(rejected/total)*100:.1f}%)\n"
        raport += f"🚫 Zablokowani uż.: {banned}\n"
        raport += f"⏳ Do ręcznej weryfikacji: {flagged}\n"
        
        await update.message.reply_text(raport, parse_mode="Markdown")
        
    except FileNotFoundError:
        await update.message.reply_text("❌ Plik logów jeszcze nie istnieje.")

async def mod_history_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Obsługa komendy /mod_history <user_id>"""
    if not context.args:
        await update.message.reply_text("❌ Użycie: `/mod_history <user_id>`\n*(Podpowiedź: użyj swojego ID Telegrama z logów)*", parse_mode="Markdown")
        return
        ...
        with open(moderation_db.USER_FILE, mode='r', encoding='utf-8') as f:
            ...
            for row in reader:
                if row['user_id'] == target_user:
                    raport = f"👤 **Historia Moderacji Użytkownika:** `{target_user}`\n"
                    raport += f"========================\n"
                    raport += f"Liczba naruszeń: **{row['total_violations']}**\n"
                    raport += f"Ostatnie naruszenie: {row['last_violation_date'][:10]}\n"
                    raport += f"Kategorie: {row['categories']}\n"
                    raport += f"Recydywista (Repeat Offender): **{'TAK 🔴' if row['is_repeat_offender'] == 'True' else 'NIE 🟢'}**\n"
                    raport += f"Shadow bany: {row['shadow_bans']}\n"
                    
                    await update.message.reply_text(raport, parse_mode="Markdown")
                    return
                    
        await update.message.reply_text(f"Brak wpisów dla użytkownika `{target_user}`. Jest całkowicie czysty!")
    except FileNotFoundError:
        await update.message.reply_text("❌ Baza użytkowników jest pusta.")

async def mod_add_feedback_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Obsługa komendy /mod_add_feedback <content_id> <APPROVE/REJECT> <komentarz>"""
    if len(context.args) < 3:
        await update.message.reply_text("❌ Użycie: `/mod_add_feedback <content_id> <APPROVE|REJECT> <Twój komentarz>`", parse_mode="Markdown")
        return
        
    content_id = context.args[0]
    override_decision = context.args[1].upper()
    comment = " ".join(context.args[2:])
    
    # Wywołujemy funkcję z moderation_db.py, którą zrobiliśmy w poprzednim kroku
    wynik = moderation_db.add_feedback(
        content_id=content_id,
        original_decision="BOT_DECISION", 
